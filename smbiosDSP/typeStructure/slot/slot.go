...
type SystemSlot struct {
	...
	DataBusWidth         string             `json:"data_bus_width,omitempty"`
	CurrentUsage         string             `json:"current_usage,omitempty"`
	Length               string             `json:"length,omitempty"`
	ID                   ID                 `json:"id,omitempty"`
	Characteristics1     string             `json:"characteristics_1,omitempty"`
	Characteristics2     string             `json:"characteristics_2,omitempty"`
...
// Parse 解析slot信息
func Parse(s *dmi.SubSmBiosStructure) (info *SystemSlot, err error) {
	info = &SystemSlot{
		Header:               s.Header,
		Designation:          s.GetString(0x0),
		Type:                 Type(s.GetByte(0x01)).String(),
		DataBusWidth:         DataBusWidth(s.GetByte(0x02)).String(),
		CurrentUsage:         Usage(s.GetByte(0x03)).String(),
		Length:               Length(s.GetByte(0x04)).String(),
		Characteristics1:     Characteristics1(s.GetByte(0x07)).String(),
		Characteristics2:     Characteristics2(s.GetByte(0x08)).String(),
		BusNumber:            Number(s.GetByte(0x0b)),
		DeviceFunctionNumber: Number(s.GetByte(0x0c)),
	}
	id, err := s.U16(0x05, 0x07)
	if err == nil {
		info.ID = ID(id)
	}
	segmentGroupNumber, err := s.U16(0x09, 0x0b)
	if err == nil {
		info.SegmentGroupNumber = SegmengGroupNumber(segmentGroupNumber)
	}
	return info, nil
}
