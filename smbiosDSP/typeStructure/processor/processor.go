...
type Processor struct {
	...
	Family2           string `json:"family_2,omitempty"`
}
...
func Parse(s *dmi.SubSmBiosStructure) (info *Processor, err error) {

	info = &Processor{
		SocketDesignation: s.GetString(0x0),
		ProcessorType:     ProcessorType(s.GetByte(0x01)).String(),
		Family:            ProcessorFamily(s.GetByte(0x02)).String(),
		Manufacturer:      s.GetString(0x3),
		ID:                ProcessorID(s.GetBytes(0x4, 0xc)).String(),
		Version:           s.GetString(0xc),
		Voltage:           ProcessorVoltage(s.GetByte(0xD)).String(),
		Status:            ProcessorStatus(s.GetByte(0x14)).String(),
		Upgrade:           ProcessorUpgrade(s.GetByte(0x15)).String(),
		...
		Family2:           ProcessorFamily(s.GetByte(0x24)).String(),
	}
	externalClock, err := s.U16(0xE, 0x10)
	if err == nil {
		info.ExternalClock = externalClock
	}
	maxSpeed, err := s.U16(0x10, 0x12)
	if err == nil {
		info.MaxSpeed = maxSpeed
	}
	currentSpeed, err := s.U16(0x12, 0x14)
	if err == nil {
		info.CurrentSpeed = currentSpeed
	}
	l1CacheHandle, err := s.U16(0x16, 0x18)
	if err == nil {
		info.L1CacheHandle = l1CacheHandle
	}
	l2CacheHandle, err := s.U16(0x18, 0x1A)
	if err == nil {
		info.L2CacheHandle = l2CacheHandle
	}
	l3CacheHandle, err := s.U16(0x1A, 0x1C)
	if err == nil {
		info.L3CacheHandle = l3CacheHandle
	}
	characteristics, err := s.U16(0x22, 0x24)
	if err == nil {
		info.Characteristics = ProcessorCharacteristics(characteristics).String()
	}
	return info, nil
}
